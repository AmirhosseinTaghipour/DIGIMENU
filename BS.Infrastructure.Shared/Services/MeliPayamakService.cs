...
{
	public class MeliPayamakService
	{
		...
		public MeliPayamakService(string username, string password, string from)
		{
			UserName = username;
			Password = password;
			From = from;
		...
		public async Task<RestResponse> SendAsync(string to, string message, bool isflash=false)
		{
		...
		public async Task<RestResponse> GetMessagesAsync(int location, string index, int count)
		{
			var values = new Dictionary<string, string>
			{
				{ "UserName", UserName },
